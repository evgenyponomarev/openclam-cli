...
export function billingStatusCmd(parent: Command) {
  parent
    .command("status")
    ...
    .action(async function (this: Command) {
      ...
      if (isJsonMode()) {
        ...
        process.stdout.write(`Available:          ${fmt(data.available_usdc_micro)}\n`);
        process.stdout.write(`Reserved:           ${fmt(data.reserved_usdc_micro)}\n`);
        process.stdout.write(`Total:              ${fmt(data.total_usdc_micro)}\n`);
        process.stdout.write(`Daily burn:         ${fmt(data.daily_burn_usdc_micro)}\n`);
        process.stdout.write(`Min reserved req:   ${fmt(data.min_reserved_required_usdc_micro)}\n`);
        process.stdout.write(`Target reserved:    ${fmt(data.target_reserved_usdc_micro)}\n`);
